...
/* Inventaire ONU — app.js (v3.5)
   - Scanner live: ROI visible + tap-to-aim + macro + zoom/torche + plein cadre périodique
   - Scanner photo: HEIC→JPEG, ZXing + BarcodeDetector + jsQR
   - Export XLSX (colonne code_scanné en texte)
   - Compteur du jour, toasts, loader, thème Pelichet
*/
...
/* Scanner params */
const DISABLE_SHARPNESS_GATE = false;   // true pour ignorer le filtre “flou”
const SHARP_MIN = 28;                   // 28–55 selon appareil/lumière
const ROI_DEFAULT = { x: 0.10, y: 0.20, w: 0.80, h: 0.60 };
const ROI_MACRO   = { x: 0.28, y: 0.32, w: 0.44, h: 0.36 };
const ROI_TAP_SIZE = { w: 0.48, h: 0.34 }; // fenêtre quand on tape
const FULLFRAME_EVERY_N = 8;            // toutes les N frames on tente plein cadre
...
const _roiMacro = { ...ROI_MACRO };
let _roiDyn = null;
let _useMacro = false;
let _noHitFrames = 0;
let _frameCount = 0;

const _noHitMacroThreshold = 35;
const _noHitZoomStepThreshold = 20;
...
async function ensureCamera(video){
  ...
  // Tap-to-aim sur vidéo + hitbox
  bindTapAimHandlers(video);
...
function ensureOverlay(){
  ...
  if (!roiBox){
    ...
    roiBox.id = "roiBox";
    overlay.appendChild(roiBox);
  }
  let hitbox = document.getElementById("scannerHitbox");
  if (!hitbox){
    hitbox = document.createElement("div");
    hitbox.id = "scannerHitbox";
    overlay.appendChild(hitbox);
  }
  return { overlay, roiBox, hitbox };
}

function paintGuides(video){
  const els = ensureOverlay(); if (!els || !video.videoWidth) return;
  const { roiBox } = els;
  const rect = video.getBoundingClientRect();
  const base = _useMacro ? _roiMacro : _roi;
  const active = _roiDyn || base;
  const left = rect.left + active.x * rect.width;
  const top  = rect.top  + active.y * rect.height;
  const w    = active.w * rect.width;
  const h    = active.h * rect.height;
  Object.assign(roiBox.style, { left: left+"px", top: top+"px", width: w+"px", height: h+"px", display: "block" });
}

function bindTapAimHandlers(video){
  ensureOverlay();
  const hit = document.getElementById("scannerHitbox");
  ...
  document.getElementById("scannerVideo")?.addEventListener("click", handler, { passive: true });
  hit?.addEventListener("click", handler, { passive: true });
}
...
function laplacianVariance(canvas){
  ...
  for (let y=1; y<h-1; y++){
    for (let x=1; x<w-1; x++){
      const c = y*w + x;
      out[c] = 4*gray[c] - gray[c-1] - gray[c+1] - gray[c-w] - gray[c+w];
    }
  }
...
async function decodeWithAll(canvas){
  // 1) Natif
  if (hasBarcodeDetector()) {
    try {
      ...
      if (res && res.length) {
        const r = res[0];
        return { text: r.rawValue || "", format: (r.format || "").toLowerCase(), path: "native" };
      }
    } catch {}
  }
  // 2) ZXing
  try {
    ...
    if (res && res.getText) return { text: res.getText(), format: String(res.getBarcodeFormat && res.getBarcodeFormat()), path: "zxing" };
  } catch {}
  // 3) jsQR
  try {
    await ensureJsQR();
    ...
    if (qr && qr.data) return { text: qr.data, format: "qr_code", path: "jsqr" };
  } catch {}
  return null;
}
...
function acceptIfStable(result){
  ...
  if (!plausible(result.format || "", text)) { _lastText = ""; _streak = 0; return null; }
...
async function startScanner(){
  ...
  _lastText = ""; _streak = 0; _useMacro = false; _noHitFrames = 0; _roiDyn = null; _frameCount = 0;
  ...
  const step = async () => {
    ...
    _decoding = true; _frameCount++;

    paintGuides(video); // maintient la boîte ROI

    try {
      // ROI
      const roi = _useMacro ? _roiMacro : _roi;
      if (drawROIFromVideo(video, roiCanvas, roi)) {
        if (!DISABLE_SHARPNESS_GATE) {
          const sharp = laplacianVariance(roiCanvas);
          if (sharp >= SHARP_MIN) {
            const res = await decodeWithAll(roiCanvas);
            if (res) {
              const accepted = acceptIfStable(res);
              if (accepted) { onCodeDetected(accepted); closeScanner(); _decoding = false; return; }
            } else { _noHitFrames++; }
          } else { _noHitFrames++; setStatus(`Flou (ROI), rapprochez/zoomez`); }
        } else {
          const res = await decodeWithAll(roiCanvas);
          if (res) {
            const accepted = acceptIfStable(res);
      ...
      // Plein cadre périodique
      if (_frameCount % FULLFRAME_EVERY_N === 0) {
        if (drawFullFromVideo(video, fullCanvas)) {
          const tryFull = async () => {
            const resF = await decodeWithAll(fullCanvas);
            if (resF) {
              const accepted = acceptIfStable(resF);
              if (accepted) { onCodeDetected(accepted); closeScanner(); _decoding = false; return true; }
            }
            return false;
          };
          if (DISABLE_SHARPNESS_GATE) {
            ...
            const sharpF = laplacianVariance(fullCanvas);
            if (sharpF >= Math.max(20, SHARP_MIN - 6)) {
              if (await tryFull()) return;
            }
          }
        }
  ...
  function schedule(){
    if ("requestVideoFrameCallback" in HTMLVideoElement.prototype) {
      video.requestVideoFrameCallback(()=> setTimeout(step, 66));
    } else {
      setTimeout(step, 66);
    }
  }
...
function stopScanner(){
  ...
  releaseCamera();
  _lastText = ""; _streak = 0; _useMacro = false; _noHitFrames = 0; _roiDyn = null; _frameCount = 0;
  const rb = document.getElementById("roiBox"); if (rb) rb.style.display = "none";
}
...
async function onPhotoPicked(ev){
  ...
  try {
    ...
    await ensureZXing(); await ensureJsQR();
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  await Promise.allSettled([ ensureZXing(), ensureJsQR() ]);
